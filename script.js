...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchMovies = async (query, page, append = false) => {
        ...
        try {
            const response = await fetch(`https://www.omdbapi.com/?s=${trimmedQuery}&page=${page}&apikey=${OMDB_API_KEY}`);
            ...
            if (data.Response === 'True') {
                ...
                movieResultsDiv.innerHTML = `<p style="text-align: center; color: var(--light-text);">No movies found for "${trimmedQuery}".</p>`;
            }
        } catch (error) {
    ...
    searchButton.addEventListener('click', (e) => {
        e.preventDefault();
        currentSearchQuery = movieSearchInput.value.trim();
        currentPage = 1;
    ...
    movieSearchInput.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            // Trim the value when setting the currentSearchQuery
            currentSearchQuery = movieSearchInput.value.trim();
            currentPage = 1;
            fetchMovies(currentSearchQuery, currentPage);
        }
